...
/*   Updated: 2023/10/23 21:23:47 by joaoalme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	calc_sprites(t_map *m, t_data *data, t_rend_sprite *r)
{
	...
	r->inv_det = 1.0 / (data->plane_x * data->dir_y - data->dir_x
			* data->plane_y);
	r->transform_x = r->inv_det * (data->dir_y * r->sprite_x - data->dir_x
			* r->sprite_y);
	r->transform_y = r->inv_det * (-data->plane_y * r->sprite_x + data->plane_x
			* r->sprite_y);
...
{
	data->tex_x = (int)(256 * (r->stripe - (-r->sprite_width / 2
					+ r->sprite_screen_x)) * TEXWIDTH / r->sprite_width) / 256;
