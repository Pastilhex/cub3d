...
/*   Updated: 2023/10/25 22:06:42 by joaoalme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_data *d, char c, t_texture *txt_ns, t_texture *txt_we)
{
	if (d->side == 0 && d->ray_dir_x > 0 && \
			d->map_ptr->world_map[(int)d->map_x][(int)d->map_y] == c)
...
static void	draw_cols2(t_data *data, int *x)
{
	...
	while (draw.y <= data->draw_end)
	{
		...
		draw_wall(data, '9', &data->txt_ptr[door], &data->txt_ptr[door]);
		draw.y++;
	}
}
	...
	if (d->map_ptr->world_map[d->map_x][d->map_y] == '9') //Closed, opening, or closing doors
		{
			if (d->side == 1)  // Door (WE)
			{
				...
				{
					if (1.0 - d->map_ptr->wall_x <= d->map_ptr->world_map[d->map_x][d->map_y])
					{
				...
				else
				{
					d->map_x += d->step_x;
					d->side = 0;
					// d->raytex = 4; //Draw door frame instead //Eu acho que aqui ele manda desenhar uma outra textura em vez da porta
					d->wallYOffset = 0; 
				}
			}
			else //side == 0 // Door (NS)
			{
				...
				if (d->side_dist_x - (d->delta_dist_x / 2) < d->side_dist_y)
				{
					...
					d->map_y += d->step_y;
					d->side = 1;
					// rayTex = 4; //Draw door frame instead //Eu acho que aqui ele manda desenhar uma outra textura em vez da porta
					d->wallXOffset = 0;
				}
			}
		}
